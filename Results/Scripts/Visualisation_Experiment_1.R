...
# -------------- PART 1: CURATED DATA ----------------
df_cur <- read.csv("Experiment_1/Curated/results.csv", encoding = "UTF-8", sep = ";")
df_cur$Model <- as.factor(df_cur$Model)

df_cur_cam <- read.csv("Experiment_1/Curated/Camembert_scores.csv", encoding = "UTF-8", sep = ";")
df_cur_flau <- read.csv("Experiment_1/Curated/Flaubert_scores.csv", encoding = "UTF-8", sep = ";")
df_cur_all_in_one <- read.csv("Experiment_1/Curated/all_in_one_scores.csv", encoding = "UTF-8", sep = ";")
...
ggplot(df_cur_words, aes(x = X, y = F1, fill = Model)) +
  ...
  theme_minimal() +
  labs(title = "Experiment 1, curated dataset: F1 scores", x = "Words", y = "Score") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
...
# We repeat this plot considering only verbs
df_cur_verbs <- df_cur_words %>%
  filter(Type == "Verb")
ggplot(df_cur_verbs, aes(x = X, y = Rand, fill = Model)) +
  geom_bar(stat = "identity", position = position_dodge(width=0.6)) +
  scale_fill_manual(values = c("#3452eb","#eb3452")) +
  theme_minimal() +
  labs(title = "Adjusted Rand Index scores for verbs", x = "", y = "Score") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
...
df_auto <- read.csv("Experiment_1/Automatic/results.csv", encoding = "UTF-8", sep = ";")
df_auto$Model <- as.factor(df_auto$Model)

df_auto_cam <- read.csv("Experiment_1/Automatic/Camembert_scores.csv", encoding = "UTF-8", sep = ";")
df_auto_flau <- read.csv("Experiment_1/Automatic/Flaubert_scores.csv", encoding = "UTF-8", sep = ";")
df_auto_all_in_one <- read.csv("Experiment_1/Automatic/all_in_one_scores.csv", encoding = "UTF-8", sep = ";")
...
ggplot(df_auto_words, aes(x = X, y = F1, fill = Model)) +
  ...
  geom_segment(data = df_auto_words, aes(x = X, xend = X, y = 0, yend = F1_no_cluster), 
               alpha = 0.8, color = "grey", linewidth = 0.8) +
  theme_minimal() +
  labs(title = "Experiment 1, Automatic dataset: F1 scores", x = "Words", y = "Score") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))

# And for the rand
ggplot(df_auto_words, aes(x = X, y = Rand, fill = Model)) +
  geom_bar(stat = "identity", position = position_dodge(width=0.6)) +
  scale_fill_manual(values = c("#3452eb","#eb3452")) +
  theme_minimal() +
  labs(title = "Experiment 1, Automatic dataset: Adjusted Rand Index scores", x = "Words", y = "Score") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
...
#-------------Comparison of curated and automatic datasets----------------
# We add a column 'Mode' to each dataset to differentiate them
df_cur_comp <- df_cur %>%
  select(X, Model, F1, V_measure, Rand, Type)
df_cur_comp$Mode <- "Curated"

df_auto_comp <- df_auto %>%
  select(X, Model, F1, V_measure, Rand, Type)
df_auto_comp$Mode <- "Automatic"

df_all <- rbind(df_cur_comp, df_auto_comp) %>%
  filter(X != "Average_nouns" & X != "Average_verbs" & X != "Global_average")


# We make a boxplot for F1 scores
ggplot(df_all, aes(x = Model, y = F1, fill = Model)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#3452eb","#eb3452")) +
  theme_minimal() +
  facet_wrap(~Mode)
# We present these results in a knitr table
df_all %>%
  group_by(Model, Mode) %>%
...
# We do the same for the v_measure scores
ggplot(df_all, aes(x = Model, y = V_measure, fill = Model)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#3452eb","#eb3452")) +
  theme_minimal() +
  facet_wrap(~Mode)
...
df_all %>%
  group_by(Model, Mode) %>%
  summarise(mean = mean(V_measure), sd = sd(V_measure)) %>%
  knitr::kable()

# We do the same for the Adjusted Rand Index scores
ggplot(df_all, aes(x = Model, y = Rand, fill = Model)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#3452eb","#eb3452")) +
  theme_minimal() +
  facet_wrap(~Mode)
...
df_all %>%
  group_by(Model, Mode) %>%
  summarise(mean = mean(Rand), sd = sd(Rand)) %>%
  knitr::kable()



# We make a barplot for camembert scores for each word, for each dataset
df_cam <- df_all %>%
  filter(Model == "Camembert")
ggplot(df_cam, aes(x = X, y = F1, fill = Mode)) +
  geom_bar(stat = "identity", position = position_dodge(width=0.6)) +
  scale_fill_manual(values = c("#3452eb","#eb3452")) +
  theme_minimal() +
  labs(title = "F1 scores", x = "Words", y = "Score") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
