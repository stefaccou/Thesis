...
# -------------- PART 1: CURATED DATA ----------------
df_cur <- read.csv("Experiment_3/Curated/BIC/results.csv", encoding = "UTF-8", sep = ";")
df_cur$Model <- as.factor(df_cur$Model)

df_cur_cam <- read.csv("Experiment_3/Curated/BIC/Camembert_scores.csv", encoding = "UTF-8", sep = ";")
df_cur_flau <- read.csv("Experiment_3/Curated/BIC/Flaubert_scores.csv", encoding = "UTF-8", sep = ";")
df_cur_all_in_one <- read.csv("Experiment_3/Curated/BIC/all_in_one_scores.csv", encoding = "UTF-8", sep = ";")
...
ggplot(df_cur_words, aes(x = X, y = F1, fill = Model)) +
  ...
  theme_minimal() +
  labs(title = "Experiment 3, curated dataset: F1 scores", x = "Words", y = "Score") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
...
df_auto <- read.csv("Experiment_3/Automatic/BIC/results.csv", encoding = "UTF-8", sep = ";")
df_auto$Model <- as.factor(df_auto$Model)

df_auto_cam <- read.csv("Experiment_3/Automatic/BIC/Camembert_scores.csv", encoding = "UTF-8", sep = ";")
df_auto_flau <- read.csv("Experiment_3/Automatic/BIC/Flaubert_scores.csv", encoding = "UTF-8", sep = ";")
df_auto_all_in_one <- read.csv("Experiment_3/Automatic/BIC/all_in_one_scores.csv", encoding = "UTF-8", sep = ";")
...
# And for Adjusted Rand Index
ggplot(df_auto_words, aes(x = X, y = Rand, fill = Model)) +
  geom_bar(stat = "identity", position = position_dodge(width=0.6)) +
  scale_fill_manual(values = c("#3452eb","#eb3452")) +
  theme_minimal() +
  labs(title = "Experiment 3, automatic dataset: Adjusted Rand Index scores", x = "Words", y = "Score") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
